...
from f1_early_stopping import EarlyStopping
...
def train(train_loader, valid_loader, test_loader, model, args):
    ...
    for epoch in range(args.num_epochs):
        ...
        #     print(
        #         f'[{epoch + 1}/{args.num_epochs}] train_loss: {train_loss.mean():.7f} valid_loss: {valid_loss.mean():.7f} elapsed time:{elapsed_time:.5f}')
        ...
        with torch.no_grad():
            ...
            results = {'precision': precision, 'recall': recall, 'f1_score':F1_score, 'roc': test_roc_ab}

        retValue = early_stopping(results, model)
        if retValue['early_stop']:
            break
    log_content = f"precision={retValue['precision']:.5f}, recall={retValue['recall']:.5f}, " \
                  f"f1-score={retValue['f1_score']:.5f}, auc-roc={retValue['roc']:.5f}\n"
    log_info(result_file, log_content, sep_flag='-')

    return retValue['roc'], retValue['precision'], retValue['recall'], retValue['f1_score']
                ...
                for sample_num in args.sampling_training:
                    ...
                    for anomaly_ratio in args.anomaly_ratio:
                        ...
                        for tr_index, va_index in kfd.split(training_data, training_labels):
                            ...
                            if args.debug_mode:
                                print(
                                    f"********* To save time, only one round test is conducted in debug mode *********\n")
                                break
