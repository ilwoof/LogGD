...
def train(train_loader, test_loader, teacher_model, student_model, args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, batch_data in enumerate(train_loader):
            ...
            embed = student_model(batch_data)
            embed_teacher = teacher_model(batch_data)
            embed_teacher = embed_teacher.detach()
            # loss = F.mse_loss(torch.sigmoid(embed.squeeze()), torch.sigmoid(embed_teacher.squeeze()), reduction='mean')

            # loss = torch.mean(calculateMahalanobis(torch.sigmoid(embed.squeeze()), torch.sigmoid(embed_teacher.squeeze())))

            loss = F.mse_loss(torch.sigmoid(embed.squeeze()), torch.sigmoid(embed_teacher.squeeze()), reduction='none').mean(dim=-1).mean(dim=0)

            # deep svdd
            # output = student_model(batch_data)
            # loss = torch.mean((output.squeeze() - c_param) ** 2)
        ...
        if (epoch + 1) % 10 == 0 and epoch > 0:
            ...
            for batch_idx, batch_data in enumerate(test_loader):
                ...
                embed = student_model(batch_data)
                embed_teacher = teacher_model(batch_data)
                scores = F.mse_loss(torch.sigmoid(embed.squeeze()), torch.sigmoid(embed_teacher.squeeze()), reduction='none').mean(dim=-1)

                # scores = calculateMahalanobis(torch.sigmoid(embed.squeeze()), torch.sigmoid(embed_teacher.squeeze()))

                # deep svdd
                # embed = student_model(batch_data)
                # scores = (embed.squeeze() - c_param) ** 2
