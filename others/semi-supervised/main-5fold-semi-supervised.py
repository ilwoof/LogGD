...
def arg_parse():
    ...
    parser.add_argument('--clip', dest='clip', default=0.01, type=float, help='Gradient clipping.')
...
def train(train_loader, valid_loader, test_loader, student_model, args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, batch_data in enumerate(train_loader):
            ...
            # loss = torch.mean((torch.sigmoid(output.squeeze()) - c_param) ** 2)      # output_dim = 1

            loss = torch.mean(torch.sum((torch.sigmoid(output.squeeze()) - c_param) ** 2, dim=-1))  # output_dim > 1
            scores = loss - r_param ** 2
            loss = r_param ** 2 + (1 / nu_param) * torch.mean(torch.max(torch.zeros_like(scores), scores))
            r_param = torch.tensor(get_radius(torch.sigmoid(output.squeeze()), nu_param)).to(device)
    ...
    for batch_idx, batch_data in enumerate(test_loader):
        ...
        output = student_model(batch_data)
        # scores = (torch.sigmoid(output.squeeze()) - c_param) ** 2  # output_dim = 1

        scores = torch.mean(torch.sum(torch.sigmoid((output.squeeze()) - c_param) ** 2, dim=-1))  # output_dim > 1
        scores = scores - r_param ** 2
            ...
            for window_size in windows:
                ...
                with open('Experiment_results.txt', 'a+') as f:
                    print(f"Dataset={params['dataset']}, Window_size={params['window_size']}, Training_mode={mode}, "
                          f"Epochs={args.num_epochs}, Anomaly_ratio={args.anomaly_ratio}\n"
                          f"Feature type={params['feature_type']}, Test_ratio={test_ratio}, "
                          f"Dataset_node_num={graphs.dataset_node_size}, Max_graph_node_num={graphs.max_graph_node_size}"
                          , file=f)
                    print('*' * 100, file=f)

                print(
                    f"Dataset={params['dataset']}, Window_size={params['window_size']}, Training_mode={mode}, "
                    f"Epochs={args.num_epochs}, Anomaly_ratio={args.anomaly_ratio}\n"
                    f"Feature_type={params['feature_type']}, Test_ratio={test_ratio}, "
                    f"Dataset_node_num={graphs.dataset_node_size}, Max_graph_node_num={graphs.max_graph_node_size}")
                print('*' * 100)

                kfd = StratifiedKFold(n_splits=kfold, random_state=args.seed, shuffle=True)

                for k, (tr_index, te_index) in enumerate(kfd.split(graphs, graphs.labels)):

                    train_index, valid_index = train_test_split(tr_index, test_size=0.3, random_state=42)

                    graphs_train_all = [graphs[i] for i in train_index]
                    graphs_valid = [graphs[i] for i in valid_index]
                    graphs_test = [graphs[i] for i in te_index]
                    ...
                    va_anomaly_num = sum([1 for graph in graphs_valid if graph[label_indicator] == 1])
                    te_anomaly_num = sum([1 for graph in graphs_test if graph[label_indicator] == 1])

                    with open('Experiment_results.txt', 'a+') as f:
                        print(f"Training size={len(graphs_train)}, "
                              f"Validation size={len(graphs_valid)}, "
                              f"Testing size={len(graphs_test)}\n"
                              f"training set anomalies={tr_anomaly_num}, "
                              f"Validation set anomalies=={va_anomaly_num}, "
                    ...
                    result = train(data_train_loader, data_valid_loader, data_test_loader, model_student, args)
