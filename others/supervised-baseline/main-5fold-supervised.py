...
def init_center_c(repre_dim, train_loader, model, eps=0.1):
    """Initialize hypersphere center c as the mean from an initial forward pass on the data."""
    n_samples = 0
    c = torch.zeros(repre_dim, device=device)
...
def train(train_loader, test_loader, teacher_model, student_model, args):
    ...
    # c_param = init_center_c(args.embedding_dim, train_loader, student_model)
    # nu_param = 0.01
    # r_param = 0.0
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch + 1) % 10 == 0 and epoch > 0:
            ...
            for batch_idx, batch_data in enumerate(test_loader):
                ...
                embed = student_model(batch_data)
                # pred = torch.where(torch.sigmoid(embed.flatten()) > 0.5, 1, 0)
                ...
                # scores = output - r_param ** 2
                # loss = r_param ** 2 + (1 / nu_param) * torch.mean(torch.max(torch.zeros_like(scores), scores))
                # em_list.append(torch.sigmoid(embed).item())
