...
class EncoderNodeFeature(nn.Module):
    ...
    def __init__(
        ...
        if num_node_type < 0:
            ...
            self.node_encoder = nn.Linear(num_node_type + 20, hidden_dim)
        else:
            self.node_encoder = nn.Embedding(num_node_type, hidden_dim)

        self.in_degree_encoder = nn.Embedding(num_in_degree, 10, padding_idx=0)
        self.out_degree_encoder = nn.Embedding(num_out_degree, 10, padding_idx=0)
        # self.in_degree_encoder = nn.Embedding(num_in_degree, hidden_dim, padding_idx=0)
        # self.out_degree_encoder = nn.Embedding(num_out_degree, hidden_dim, padding_idx=0)

        torch.nn.init.kaiming_normal_(self.node_encoder.weight.data)
        torch.nn.init.zeros_(self.node_encoder.bias.data)
    ...
    def forward(self, batched_data):
        ...
        node_feature = torch.cat([x, self.in_degree_encoder(in_degree), self.out_degree_encoder(out_degree)], dim=-1)
        node_feature = self.node_encoder(node_feature)
...
class GRPENetwork(nn.Module):
    def __init__(
        ...
        self.encode_node = EncoderNodeFeature(num_node_type, num_in_degree=100, num_out_degree=100, hidden_dim=d_model)
        ...
        self.linear = nn.Linear(2 * d_model, out_dim)
