...
class GatedGCNNetwork(nn.Module):
    def __init__(
        self,
        out_dim=1,
        num_layer=6,
        ...
        for m in self.modules():
            if isinstance(m, GatedGraphConv):
...
class EncoderLayer(nn.Module):
    def __init__(self, output_size, num_layers):
        ...
        self.self_cov_layers = GatedGraphConv(output_size, num_layers)

    def forward(self, data):
        x, edge_index, edge_weight = data.x, data.edge_index, data.edge_weight
        x = self.self_cov_layers(x, edge_index, edge_weight)
        x = F.relu(x)
