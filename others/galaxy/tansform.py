...
class LogCollator(object):
    ...
    def __call__(self, batch):
        ...
        in_degree = [b["in_degree"] for b in batch]
        out_degree = [b["out_degree"] for b in batch]
        ...
        for n, d, ei, ea, ew, ind, outd in zip(node, distance, edge_index, edge_attr, edge_weight, in_degree, out_degree):
            m = torch.zeros(max_num_node, dtype=torch.bool)
            m[n.shape[0] :] = 1

            new_n = -torch.ones((max_num_node, n.shape[1]), dtype=torch.float32)
            new_n[: n.shape[0]] = n
            ...
            new_ew = torch.zeros((max_num_node, max_num_node), dtype=torch.long)
            new_ew[ei[0], ei[1]] = ew

            new_ind = torch.zeros(max_num_node, dtype=torch.long)
            new_ind[: n.shape[0]] = ind
            new_outd = torch.zeros(max_num_node, dtype=torch.long)
            new_outd[: n.shape[0]] = outd
