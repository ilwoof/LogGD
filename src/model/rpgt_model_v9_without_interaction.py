...
class GRPENetwork(nn.Module):
    def __init__(
        ...
        self.query_hop_emb = nn.Embedding(max_hop + 2, 1)
        self.key_hop_emb = nn.Embedding(max_hop + 2, 1)
        self.value_hop_emb = nn.Embedding(max_hop + 2, 1)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        spatial_bias = query_hop_emb(distance.unsqueeze(1).repeat(1, self.num_heads, 1, 1)).sum(dim=-1)

        x = torch.matmul(q, k.transpose(2, 3))
        x = x * self.scale + spatial_bias
